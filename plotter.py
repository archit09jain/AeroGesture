...
"""
	make sure the length of the file is same for each gesture
	adjust the frequency and the time for which the gesture need to be recorded
	try taking atleast 100 samples
"""

## returns the indiviual component acc
def getFeatures(path):
		...
		# standard deviation alg each axis
		sx,sy,sz = np.std(x),np.std(y),np.std(z)

		dalpha = 0
		## removal of unnecssary handshaking at the beginning and at the end of the gesture
		## truncation factor
		dx = dalpha *  sqrt(sx)
		dy = dalpha * sqrt(sy)
		dz = dalpha * sqrt(sz)
